...
type evaluator struct {
	...
	// quiet counts the unevaluated operands enclosing the current one: the
	// right side of a short circuit that already has its answer, and the arm
	// of a ?: that was not taken. Nothing inside one reports.
	//
	// The decision has to be made before the operand is evaluated rather
	// than unwound afterwards, because a diagnostic that has been appended
	// has been reported — `#if 0 && 1/0` is not a division by zero that gets
	// forgiven, it is not a division at all.
	quiet int
}
...
// defined() chain), and objv follows, under a named warning so the
	...
	for _, t := range line {
		if t.Is("defined") {
			p.warn("expansion-defined", t.Site(),
				"\"defined\" produced by macro expansion is not portable; objv evaluates it as the operator, matching gcc and clang")
...
// unevaluated runs f with reporting off when skip is set, and leaves e.bad as
// it found it: an operand nobody evaluated cannot make the expression wrong.
func (e *evaluator) unevaluated(skip bool, f func() value) value {
	if !skip {
		return f()
	}
	save := e.bad
...
func (e *evaluator) binary(min int) value {
	...
	for {
		...
		// and `0 && 1/0` folds without reporting. The right side is known to
		// be unevaluated before it is walked, which is what keeps its
		// diagnostics from being appended in the first place.
		if t.Kind == token.LAND || t.Kind == token.LOR {
			skip := (t.Kind == token.LAND && !lhs.nonzero()) ||
				(t.Kind == token.LOR && lhs.nonzero())
			rhs := e.unevaluated(skip, func() value { return e.binary(pr + 1) })
			n := uint64(0)
...
// same source text. The suffixes it accepts are the ones §2.3 admits and no
// others; a spelling the scanner would refuse to classify has no business
// carrying a value here.
