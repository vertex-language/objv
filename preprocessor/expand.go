...
type stream struct {
	...
	// top marks the stream that feeds the output: the one _Pragma may act
	// on. A closed sequence — a macro argument, an #if line — is not it. A
	// _Pragma there is left standing and is acted on later, when the
	// replacement list it was substituted into is rescanned here.
	top bool
}
...
func (p *Preprocessor) expandInto(s *stream, out []Token) []Token {
	for {
		...
		if t.Kind == token.IDENT && s.top {
			if t.Text() == "_Pragma" {
				if pragma, handled := p.pragmaOperator(s, t); handled {
					out = append(out, pragma...)
					continue
				}
			}
			if answer, handled := p.operatorInText(s, t); handled {
				if answer.Kind != token.ILLEGAL {
...
func (p *Preprocessor) subst(m *Macro, is []Token, ap [][]Token, hs *HideSet, use Site, outer *Expansion) []Token {
	...
	for i := 0; i < len(is); i++ {
		...
		// # parameter, and Objective-C's @# parameter.
		//
		// An @ immediately before the # makes the result a string object
		// rather than an array of characters (§2.4). The two tokens are one
		// operator in that spelling, so the @ is taken back off the output
		// and the whole thing is minted as the single OBJC_STRING_LIT the
		// grammar says an ObjectStringLiteral is.
		if t.Kind == token.HASH && i+1 < len(is) {
			if n, ok := arg(is[i+1]); ok {
				object := len(os) > 0 && os[len(os)-1].Kind == token.AT && !t.Spaced()
				var st Token
				if object {
					at := os[len(os)-1]
					os = os[:len(os)-1]
					st = p.gen.StringizeObject(ap[n])
					st.Flags = at.Flags & token.FlagAdjacent
				} else {
