...
// §5.1's one-storage-class rule and C11 §6.7.2's type specifier multiset
// table. On an invalid multiset it reports once and yields int, so
// construction always produces a type.
func BuildSpecs(unit *token.File, specs ast.DeclSpecs, r Resolver) Spec {
	var sp Spec
	var quals Qual
	var life Lifetime
	var null Nullability
	var kws []token.Kind
	var other Type // struct/union/enum/typedef/object/_Atomic() specifier
	...
	for _, s := range specs {
		...
		case *ast.KeywordSpec:
			...
			case token.WEAK:
				life = LifeWeak
			case token.UNSAFE_UNRETAINED:
				life = LifeUnsafeUnretained
			case token.AUTORELEASING:
				life = LifeAutoreleasing
			case token.NONNULL:
				null = NullNonnull
			case token.NULLABLE:
				null = NullNullable
			case token.NULL_UNSPECIFIED:
				null = NullUnspecified
			case token.AUTO_TYPE:
				// Not a member of the multiset table: it names no type of
				// its own, so it is recorded and kept out of kws.
				sp.Auto = true
		...
		case *ast.TypeofType:
			...
			other, otherNode = t, s
		case *ast.PtrauthSpec:
			// §5.6's pointer authentication. Its arguments are the
			// implementation's, and objv emits no signed pointers, so the
			// qualifier is accepted and carries no type information.
		case *ast.AttrSpec:
			sp.Attrs = append(sp.Attrs, s.Attrs...)
		}
	}

	// _Thread_local combines only with static and extern (C11 §6.7.1p3).
	...
	// §5.6: __kindof qualifies an object pointer and nothing else. The
	// qualifier is written before the class name, so it arrives here
	// applied to the interface type; it belongs on the pointer the
	// declarator will build, which is where the declarator puts it.
	if quals&QKindOf != 0 && Unqualify(base).Kind() != ObjectKind && !IsObjectPointer(base) {
		r.Report(anchor(specs), "__kindof requires an Objective-C object type")
		quals &^= QKindOf
	}

	t := Qualify(base, quals)
	t = WithLifetime(t, life)
	sp.Type = WithNullability(t, null)
	return sp
}
...
// multiset canonicalizes a basic-keyword multiset and looks it up in C11
// §6.7.2p2's table.
...
type builder struct {
	...
	// against the element type to name in the report. It is a map and not a
	// report on the spot because whether it is one depends on a fact the
	// derivation does not have yet — see the loop in BuildDeclarator.
	incomplete map[*Array]Type
}

// BuildDeclarator derives a type from base through a declarator tree,
// reporting derivation constraints as it goes. It returns the derived type
// and the declared identifier (nil for an abstract declarator). param
// enables the parameter-only forms and checks.
//
...
func BuildDeclarator(unit *token.File, base Type, d ast.Declarator, param bool, r Resolver) (Type, *ast.Ident) {
	...
	// [static …] and [*] belong only to the outermost array derivation of a
	// parameter — the one the parameter adjustment rewrites.
	...
	// An array wants a complete element type, and the outermost array of a
	// parameter is the one place there is no array to want it for: the same
	// adjustment rewrites it to a pointer to the element, so
	// `void f(struct S a[])` is `void f(struct S *a)` and a forward
...
func (b *builder) build(base Type, d ast.Declarator) (Type, *ast.Ident) {
	...
	case *ast.PtrDeclarator:
		q, life, null := b.quals(d.Quals)
		// §5.6's qualifiers are written before the class name, so they
		// arrive on the interface type; what they qualify is the pointer to
		// it, which is this one. `__weak NSString *s` makes the *pointer*
		// weak — there is no other pointer for it to mean — and moving them
		// here is what makes it and `NSString *__weak s` the same type, as
		// clang has them.
		var moved Lifetime
		var movedNull Nullability
		base, q, moved, movedNull = liftObjectQuals(base, q)
		if life == LifeNone {
			life = moved
		}
		if null == NullNone {
		...
		t := &Pointer{Elem: base}
		return b.build(WithNullability(WithLifetime(Qualify(t, q), life), null), d.Inner)

	case *ast.BlockPtrDeclarator:
		// §5.7's block pointer. A declarator derives inside-out, so by the
		// time the '^' is reached the parameter list has already made the
		// function type this points at: `void (^b)(int)` arrives here as
		// void(int).
		q, life, null := b.quals(d.Quals)
		fn, ok := Unqualify(base).(*Func)
		if !ok {
			// `void (^b);` — a block pointer to something that is not a
			// function, which is not a type the language has.
			b.r.Report(d, "block pointer to non-function type "+base.String())
			fn = &Func{Ret: base, Proto: true}
		}
		blk := WithNullability(WithLifetime(Qualify(&Block{Sig: fn}, q), life), null)
		return b.build(blk, d.Inner)
	...
	case *ast.ArrayDeclarator:
		...
		case under.Kind() == ObjectKind:
			b.r.Report(d, "array of the interface type "+under.String()+
				"; an array of objects holds pointers")
			arr.Elem = &Pointer{Elem: base}
		...
		q, _, _ := b.quals(d.Quals)
		return b.build(Qualify(arr, q), d.Inner)

	case *ast.FuncDeclarator:
		fn := &Func{Ret: base}
		switch under := Unqualify(fn.Ret).Kind(); under {
		case FuncKind:
...
// liftObjectQuals takes the qualifiers that belong to a pointer off the
// interface type they were written on. It does nothing to any other type: a
// `const int *` is a pointer to a const int, and only an object's ownership,
// nullability and __kindof describe the pointer instead.
func liftObjectQuals(base Type, q Qual) (Type, Qual, Lifetime, Nullability) {
	if Unqualify(base).Kind() != ObjectKind {
		return base, q, LifeNone, NullNone
	}
	life, null := LifetimeOf(base), NullabilityOf(base)
	if QualsOf(base)&QKindOf != 0 {
		q |= QKindOf
		base = WithoutQual(base, QKindOf)
	}
	if life != LifeNone || null != NullNone {
		base = Unqualify(base)
		base = Qualify(base, QualsOf(base))
	}
	return base, q, life, null
}

// quals reads a qualifier list from a declarator's brackets or pointer.
func (b *builder) quals(list ast.DeclSpecs) (Qual, Lifetime, Nullability) {
	var q Qual
	life := LifeNone
	null := NullNone
	for _, s := range list {
...
func (b *builder) buildParams(fn *Func, d *ast.FuncDeclarator) {
	if len(d.Params) == 0 {
		// f() and f(a, b): parameters unspecified; the identifier-list
		// names, if any, get their types from the definition's declaration
		// list — the analyzer's job.
		return
	}
	fn.Proto = true
	...
	for _, p := range d.Params {
		...
		t, id := BuildDeclarator(b.unit, sp.Type, p.Decl, true, b.r)
...
// AdjustParam applies C11 §6.7.6.3p7–8: a parameter of array type becomes a
// pointer to the element type, carrying the qualifiers written inside the
// brackets; a parameter of function type becomes a pointer to it.
//
// A block pointer is already a pointer and is not adjusted, and neither is
// an object pointer: both are what they are written as.
...
// complete reports whether t can be an array element or a declared object's
// type: not void, not an incomplete record, enum or class, not an incomplete
// array, and not an interface type — a program holds a pointer to an object,
// never an object.
...
// Complete is the exported form of the element/object completeness test, for
// the analyzer's declared-object checks.
func Complete(t Type) bool {
	return (&builder{}).complete(t)
}

// Selector builds §4.7's selector from a method's keyword pieces: the names
// with their colons, joined. A unary method's selector is its one name.
//
// It is here rather than in the analyzer because everything downstream keys
// on the string — the runtime's dispatch table, method lookup, the
// diagnostic that says a method was not found — and a selector assembled
// twice is a selector that can differ.
func Selector(pieces []string, keyword bool) string {
	if !keyword {
		if len(pieces) == 0 {
			return ""
		}
