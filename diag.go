...
type Diagnostic struct {
	...
	// Notes are secondary positions: a macro's definition site, a previous
	// declaration, the @interface a method was declared in.
	Notes []Note
}
...
// A DiagnosticError is the error a compile returns when the *program* is
// wrong, as opposed to the invocation. Errors of every other kind mean objv
// could not run: an unknown target, an unreadable file, a module that fails
// verification.
//
...
// space — scanner, parser, analyzer, lower — with the file that owns them, so
// they can be rendered beside the ones this package returns.
...
func mapDiagnostics(f *token.File, m srcMap, diags []token.Diagnostic) []Diagnostic {
	...
	// System-header warnings already reported, by header and message.
	seen := map[string]bool{}
	for _, d := range diags {
		site := preprocessor.Site{Origin: own, Pos: d.Pos, End: d.End}
		if m != nil {
			// Clamped, because a span may end one past the last byte: a
			// parser reporting "expected ')'" at end of input sites it on
			// the EOF token, whose end is the position after a file that
			// has no position after it.
		...
		// not each #import of it. Until here only phase 4 applied the rule,
		// because the parser and the analyzer run over the reparse bridge
		// and cannot see an Origin; here it is known.
		//
		// It matters more in Objective-C than in C. One
		// `#import <Foundation/Foundation.h>` reaches some nine hundred
		// headers, and a warning repeated per inclusion would bury the
		// program under its own frameworks.
