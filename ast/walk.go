...
// Walk traverses the tree rooted at n in depth-first order. Children are
// discovered by reflection over exported fields in source order, so a new
// field on a node traverses without this file changing — which matters in a
// tree this wide, where the alternative is ninety hand-written switch arms
// that fall out of date one at a time.
//
// Typed nils are skipped, as are Span fields and fields tagged `ast:"-"`.
//
// This is the reference implementation; if it shows up in a profile, generate
// the switch and keep this as the oracle.
...
func collect(v reflect.Value, out *[]Node) {
	...
	case reflect.Slice:
		if v.Type().Elem() == spanType {
			return // StringLit.Segs and the like: extents, not children
		}
		for i := 0; i < v.Len(); i++ {
