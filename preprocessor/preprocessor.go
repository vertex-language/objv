...
type Preprocessor struct {
	...
	// internal is set while the compiler's own predefines are being
	// installed. §6.10.8p2 forbids *the program* from defining __STDC__ and
	// its kin; the compiler that supplies them is not the program, and this
	// is the one place the check does not apply.
	internal bool
...
func (p *Preprocessor) Run(f *token.File) ([]Token, []Diagnostic) {
	...
	for _, inc := range p.cfg.PreIncludes {
		p.include(r, inc, false, Site{Origin: org, Pos: f.Pos(0), End: f.Pos(1)}, incInclude)
	}
	p.run(r)
...
// Macros exposes the table after a run, for `objv env` and for tests.
...
func (p *Preprocessor) expandText(r *reader) {
	s := &stream{top: true, more: func() (Token, bool) {
		t, ok := r.peek()
...
// topReader is the file currently being read. _Pragma consults it, because
// `_Pragma("once")` is `#pragma once` and says the same thing about the same
// file.
func (p *Preprocessor) topReader() *reader {
	if len(p.stack) == 0 {
		return nil
	}
...
//
// ScanPP differs from the ordinary mode in four ways, all of them because a
// pp-token is not yet a token: pp-numbers keep their spans without being
...
// an @ on an identifier that is not a directive is deferred with them, and a
// line-opening '#' is returned as HASH rather than swallowed as trivia.
