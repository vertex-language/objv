...
func (g *Gen) Stringize(arg []Token) Token {
	return g.stringize(arg, "", token.STRING_LIT)
}

// StringizeObject is `@#x`, the Objective-C spelling of the same operator:
// the same characters, headed by an @, as one OBJC_STRING_LIT.
//
// It is minted as one token rather than as an @ and a string because that is
// what §2.4 makes an ObjectStringLiteral — a single token — and because the
// idiom exists precisely so a macro can hand a caller an NSString:
//
//	#define OBJV_STR(x) @#x
//	[dict setObject:v forKey:OBJV_STR(count)];
//
// A string object built any other way would be an array of characters, which
// is not what the caller asked for and not what -setObject:forKey: takes.
func (g *Gen) StringizeObject(arg []Token) Token {
	return g.stringize(arg, "@", token.OBJC_STRING_LIT)
}

func (g *Gen) stringize(arg []Token, prefix string, kind token.Kind) Token {
