...
// through phase 4 when it should be, with the include graph flattened into
// one position space.
//
...
// Preprocess runs phase 4 and returns its output as Objective-C source — the
// artifact `--emit mi` writes.
//
// An input that was already preprocessed comes back byte for byte: the
// guarantee is that the output re-enters as .mi input and produces the same
// program, and both a dropped #pragma and re-spaced whitespace break it.
//
// Diagnostics come back whether or not they are errors. Output produced
// alongside an error is what phase 4 got to before giving up; a caller
// writing an artifact should check HasErrors first.
...
func (c *Compiler) Parse(in Input, mode parser.Mode) (*ast.File, []Diagnostic, error) {
	f, m, diags, err := c.source(in)
	if err != nil {
		return nil, nil, err
	}
	tree, parseDiags := parser.ParseFile(f, mode)
	diags = append(diags, mapDiagnostics(f, m, parseDiags)...)
...
func (c *Compiler) frontend(in Input) (*unit, error) {
	...
	info, checkDiags := analyzer.Check(f, tree, t.Model(), mode)
	...
	return &unit{
		in: in, target: t, file: f, smap: smap, tree: tree, info: info,
...
// afterwards, so callers copy what they need first — which for a unit that
// has already produced its artifact is nothing.
func (u *unit) release() {
	if u.tree != nil {
		u.tree.Release()
	}
...
// any: that is the difference Preprocess needs, since a .mi file passes
...
func (c *Compiler) stream(in Input) (raw *token.File, toks []preprocessor.Token, ran bool, diags []Diagnostic, err error) {
	...
	// look for their quoted imports beside themselves. config is shared and
...
// source is stream plus the reparse bridge: the unit the parser reads, and
// the map from its positions back to the ones the user wrote.
func (c *Compiler) source(in Input) (*token.File, srcMap, []Diagnostic, error) {
	raw, toks, ran, diags, err := c.stream(in)
	if err != nil {
		return nil, nil, nil, err
	}
	if !ran {
		// Already preprocessed: the positions are the file's own.
		return raw, nil, diags, nil
	}
	f, m, err := reparse(in, toks)
	if err != nil {
		return nil, nil, nil, err
	}
	return f, m, diags, nil
}

// reparse is the bridge until parser.ParseTokens exists.
//
// Phase 4's output spans every file the include graph reached; ParseFile
// takes one *token.File. So the stream is printed and re-scanned as .mi
// input — the round trip Preprocess already promises, used as plumbing. The
// cost is the position map, which is why one is recorded.
func reparse(in Input, toks []preprocessor.Token) (*token.File, srcMap, error) {
	var b strings.Builder
	var m srcMap
	if err := printTokens(&b, toks, printOpts{srcMap: &m}); err != nil {
		return nil, nil, err
	}
	return token.NewFile(in.name(), []byte(b.String())), m, nil
}
